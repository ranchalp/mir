...
func IncludeCreatingCertificates(
	...
	mempooldsl.UponNewBatch(m, func(txIDs []t.TxID, txs []*requestpbtypes.Request, context *requestBatchFromMempoolContext) error {
		...
		mppbdsl.RequestBatchID(m, mc.Mempool, txIDs, params.PrevBatchID, &requestIDOfOwnBatchContext{context.reqID, txs})
		return nil
	})
	...
	mempooldsl.UponBatchIDResponse(m, func(batchID t.BatchID, context *requestIDOfOwnBatchContext) error {
		...
		// TODO: add persistent storage for crash-recovery.
		eventpbdsl.SendMessage(m, mc.Net, mscpbmsgs.RequestSigMessage(mc.Self, context.txs, params.PrevBatchID, context.reqID), params.AllNodes)
		return nil
	})
	...
	dsl.UponCondition(m, func() error {
		...
		for reqID, requestState := range state.RequestState {
			if len(requestState.sigs) >= params.F+1 {
				...
				eventpbdsl.SendMessage(m, mc.Net, mscpbmsgs.CertMessage(mc.Self, cert), params.AllNodes)
	...
	// When received a Cert Message, verify the certificate
	mscpbdsl.UponCertMessageReceived(m, func(from t.NodeID, cert *mscpbtypes.Cert) error {
		if from != params.Source {
			return nil
			//TODO check here that the cert message received is the same as the one stored (always true unless equivocation f>n/3)
		}
		// TODO THIS IS NOT ENOUGH VERIFICATION
		// the receiver needs to also verify that the batch is properly signed with the proper prevBatchID
		// For this, the receiver either needs the entire batch (or the signature must be on the hash
		// of attaching the prevBatchID to the BatchID, and that should be the actual (i.e. new) BatchID)
		//call verifyCert but keep nodeID in context
		apbdsl.VerifyCert(m, mc.Self, &apbtypes.Cert{Type: &apbtypes.Cert_Msc{Msc: cert}}, cert)
		return nil
	})

	apbdsl.UponCertVerified(m, func(valid bool, err string, cert *mscpbtypes.Cert) error {
		if !valid {
			// Ignore invalid certificate.
			return nil
		}
		params.DeliveredCert = cert
		//Notify provider that this is finished
		apbdsl.CertReceived(m, mc.AvailabilityProvider, &apbtypes.Cert{Type: &apbtypes.Cert_Msc{Msc: cert}}, params.Source, mc.Self, params.WorkerCount)
		return nil
	})

	// When receive a request for a signature, compute the ids of the received transactions.
	mscpbdsl.UponRequestSigMessageReceived(m, func(from t.NodeID, txs []*requestpbtypes.Request, prevBatch t.BatchID, reqID RequestID) error {
		if !reflect.DeepEqual(prevBatch, params.PrevBatchID) {
			//ignore
			return nil
		}
		mempooldsl.RequestTransactionIDs(m, mc.Mempool, txs, &computeIDsOfReceivedTxsContext{from, txs, reqID, prevBatch})
		return nil
	})
	...
	mempooldsl.UponTransactionIDsResponse(m, func(txIDs []t.TxID, context *computeIDsOfReceivedTxsContext) error {
		mppbdsl.RequestBatchID(m, mc.Mempool, txIDs, context.prevBatch,
